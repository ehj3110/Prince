...
class AdhesionMetricsCalculator:
    ...
    def _calculate_metrics(self, 
        ...
        # Step 2: Find propagation end using second derivative maximum with position constraint
        prop_end_idx = self._find_propagation_end_second_derivative(
            times, positions, smoothed_force, peak_idx, motion_end_idx)
    ...
    def _find_propagation_end_second_derivative(self, 
        ...
        """
        Find propagation end using second derivative maximum method with position-based constraint.
        
        Args:
            times: Time array
            positions: Position array (decreases during upward motion)
            smoothed_force: Smoothed force data
            peak_idx: Index of peak force
            motion_end_idx: Optional end of motion constraint
            
        Returns:
            Index of propagation end
        """
        ...
        # Add position-based constraint to avoid machine shake during direction reversal
        position_constrained_end_idx = self._find_position_constraint_end(positions, peak_idx, motion_end_idx)
        motion_end_idx = min(motion_end_idx, position_constrained_end_idx)
    ...
    def _find_position_constraint_end(self, positions: np.ndarray, peak_idx: int, motion_end_idx: int) -> int:
        """
        Find the index where position constraint should end propagation search.
        
        During peel tests, the stage moves up (position decreases) until reaching the step distance,
        then reverses direction down. Machine shake during reversal can create false force peaks.
        
        Args:
            positions: Position array (mm) - decreases during upward motion
            peak_idx: Index of peak force
            motion_end_idx: Current motion end constraint
            
        Returns:
            Index where propagation search should end (80% of step distance)
        """
        try:
            # Get position at peak
            peak_position = positions[peak_idx]
            
            # Look ahead to find the minimum position (maximum step distance)
            search_end = min(motion_end_idx, len(positions))
            future_positions = positions[peak_idx:search_end]
            
            if len(future_positions) < 10:
                return motion_end_idx  # Not enough data, use original constraint
            
            # Find the minimum position (where stage reaches maximum step distance)
            min_position_relative_idx = np.argmin(future_positions)
            min_position = future_positions[min_position_relative_idx]
            
            # Calculate step distance (how far the stage moves up from peak)
            step_distance = peak_position - min_position  # Should be positive
            
            if step_distance <= 0.01:  # Less than 0.01mm step, probably no real motion
                return motion_end_idx
            
            # Find where we reach 80% of the step distance
            target_position = peak_position - (0.8 * step_distance)
            
            # Find the index where position first crosses this threshold
            for i, pos in enumerate(future_positions):
                if pos <= target_position:
                    constraint_idx = peak_idx + i
                    print(f"    Position constraint: Stop search at 80% step distance "
                          f"(pos {target_position:.3f}mm, idx {constraint_idx})")
                    return constraint_idx
            
            # If we never reach 80%, use the original constraint
            return motion_end_idx
            
        except Exception as e:
            warnings.warn(f"Position constraint calculation failed: {e}. Using original motion constraint.")
            return motion_end_idx
