...
## Layer Stage Detection

### **Position-Based Stage Boundaries**
- **Purpose:** Segment continuous print data into individual layer cycles
- **Method:** Track stage position changes to identify printing stages
- **Implementation:**
  1. Start from identified force peaks
  2. Search backwards to find lifting stage start (position change > 0.01mm)
  3. For layer end boundary:
     - If not last layer: Use start of next layer's lifting motion
     - If last layer: Search forward up to 20 seconds or until position stabilizes
  4. Buffer zones added (50 samples) to capture complete stage transitions

### **Printing Stage Definitions**
1. **Exposure Stage**
   - Position: Stationary
   - Force: Baseline, minimal variation
   - Ends when upward motion detected

2. **Lifting Stage**
   - Position: Moving upward
   - Force: Contains critical peel force data
   - Contains the adhesion force profile

3. **Retraction Stage**
   - Position: Moving downward
   - Force: Return to baseline
   - Ends at new layer height

4. **Pause Stage**
   - Position: Stationary at new height
   - Force: Baseline readings
   - Ends when next exposure begins
