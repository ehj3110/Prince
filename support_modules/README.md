...
0.245,10.000,0.018,Lift-Stage1
```
...
**Usage:**
```python
from support_modules.adhesion_metrics_calculator import calculate_adhesion_metrics_from_data
...
**Usage:**
```python
from support_modules.two_step_baseline_analyzer import detect_baseline_two_step
...
### `AutoHomeRoutine.py`
**Purpose:** Automated Z-axis homing using force feedback

**Algorithm:**
1. Move stage downward slowly
2. Monitor force gauge continuously
3. Detect contact when force exceeds threshold
4. Stop immediately and set home position
5. Retract slightly to release pressure

**Key Features:**
- Force-based detection (more reliable than limit switch)
- Configurable approach speed and threshold
- Threaded execution (non-blocking)
- Status reporting to main GUI

**Usage:**
```python
auto_homer = AutoHomer(
    axis=stage_axis,
    force_manager=force_manager,
    status_callback=update_status
)
auto_homer.start_auto_home()
```
...
**Purpose:** Custom metadata entry for experimental parameters
...
**Output:**
Saves experimental conditions to JSON file in print directory for later reference.

---

## Utilities

### `USBCoordinator.py` & `dlp_phidget_coordinator.py`
...
3. **`AutoHomeRoutine.py`** - Core routine
