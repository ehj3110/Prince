...
class SensorDataWindow:
    ...
    def __init__(self, master_window, zaber_axis_ref, main_app_status_callback, prince_main_app_ref, existing_force_gauge_manager=None):
...
'''
        ...
        # Use existing ForceGaugeManager if provided, otherwise create new one
        if existing_force_gauge_manager:
            ...
            print("SensorDataWindow: Creating NEW ForceGaugeManager (WARNING: May cause connection issues!)")
        ...
        # Work of Adhesion checkbox removed for RED lab (not needed)
    ...
    def _setup_default_logging_paths(self):
        ...
        # Extract directory from txt file path (RED Lab uses txt file path, not directory)
        if main_app_txt_file_path and os.path.isfile(main_app_txt_file_path):
            main_app_image_dir_val = os.path.dirname(main_app_txt_file_path)
        elif main_app_txt_file_path and os.path.isdir(main_app_txt_file_path):
            # Fallback: if it's already a directory (Prince style)
            main_app_image_dir_val = main_app_txt_file_path
        else:
            main_app_image_dir_val = main_app_txt_file_path  # Let validation below handle it
    ...
    def add_window_to_active_file(self):
        ...
        if not self.current_logging_windows_file or \
           ...
           not os.path.isdir(os.path.dirname(self.current_logging_windows_file)):
            ...
            if self.auto_log_enabled_var.get():
                ...
                if not self.current_logging_windows_file or \
                   not os.path.exists(self.current_logging_windows_file) or \
                   not os.path.isdir(os.path.dirname(self.current_logging_windows_csv_path)):
        ...
        try:
            ...
            traceback.print_exc()
            messagebox.showerror("File Error", f"Could not write to {os.path.basename(self.current_logging_windows_csv_path)}: {e}", parent=self.sensor_window)
