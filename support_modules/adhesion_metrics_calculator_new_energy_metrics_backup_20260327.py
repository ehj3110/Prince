...
class AdhesionMetricsCalculator:
    ...
    def _calculate_metrics(self, 
        ...
        # Step 7: Calculate retained energy metrics + brittleness
        ...
        # Step 11: Optional retraction metrics from paired retraction phase data.
    ...
    def _calculate_work_metrics(self, 
                              ...
                              peak_idx: int,
                              prop_end_idx: int,
                              contact_area_m2: float) -> Dict:
        """
        Calculate retained energy metrics and brittleness.
        
        Retained metrics from the previous energy set:
        - work_of_adhesion_mJ (raw force integral)
        - work_of_adhesion_corrected_mJ (baseline-corrected force integral)
        - energy_release_rate_G_J_per_m2 = corrected_work_J / contact_area_m2

        Added metric:
        - brittleness_factor = (initiation corrected work from lift-start to peak)
                               / (corrected work from lift-start to propagation-end)
        """
        results = {}

        # Integrate over the physically defined peel region: pre-initiation to propagation-end.
        start_idx = max(int(pre_init_idx), 0)

        # Integrate from lift-start to propagation-end.
        peel_positions = positions[start_idx:prop_end_idx + 1]
        peel_forces = forces[start_idx:prop_end_idx + 1]
        ...
        try:
            ...
            # Baseline-corrected work. Enforce nonnegative output for manuscript reporting.
            work_corrected_raw_J = np.trapezoid(peel_forces_corrected, peel_positions_m)
            if work_corrected_raw_J < 0:
                warnings.warn(
                    "Baseline-corrected work was negative; clipping to 0. "
                    "Check baseline/phase boundaries for this layer."
                )
            work_corrected_J = max(work_corrected_raw_J, 0.0)
            results['work_of_adhesion_corrected_mJ'] = work_corrected_J * 1000

            # Energy release rate from baseline-corrected work over contact area.
            if contact_area_m2 > 0:
                results['energy_release_rate_G_J_per_m2'] = max(work_corrected_J / contact_area_m2, 0.0)
            else:
                warnings.warn("contact_area must be > 0 for energy release rate. Returning G=0.")
                results['energy_release_rate_G_J_per_m2'] = 0.0

            # Brittleness = initiation corrected work / total corrected work.
            init_positions = positions[start_idx:peak_idx + 1]
            init_forces_corrected = forces[start_idx:peak_idx + 1] - baseline
            if len(init_positions) >= 2:
                init_positions_m = init_positions / 1000.0
                if np.any(np.diff(init_positions_m) <= 0):
                    sorted_init = np.argsort(init_positions_m)
                    init_positions_m = init_positions_m[sorted_init]
                    init_forces_corrected = init_forces_corrected[sorted_init]
                initiation_work_corrected_J = np.trapezoid(init_forces_corrected, init_positions_m)
            else:
                initiation_work_corrected_J = 0.0

            if abs(work_corrected_J) > 1e-15:
                results['brittleness_factor'] = initiation_work_corrected_J / work_corrected_J
            else:
                results['brittleness_factor'] = 0.0
