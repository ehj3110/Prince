...
for the Prince printing system. Extracted from Prince_Segmented.py to improve
...
class SessionManager:
    ...
    def save_gui_state(self):
        ...
        try:
            state = {
                ...
                # Window states
                'sensor_window_open': (self.parent.sensor_data_window_instance is not None and 
                                      self.parent.sensor_data_window_instance.sensor_window.winfo_exists()),
                'exp_conditions_window_open': (self.parent.exp_conditions_window is not None and 
                                              self.parent.exp_conditions_window.window.winfo_exists()),
                
                # Sandwich settings
                'enable_sandwich_precalib': self.parent.enable_sandwich_precalib.get(),
                'enable_adaptive_sandwich': self.parent.enable_adaptive_sandwich.get(),
                'enable_scaled_force_sandwich': self.parent.enable_scaled_force_sandwich.get(),
                'sandwich_gap': self.parent.t_sandwich_gap.get() if hasattr(self.parent, 't_sandwich_gap') else '0.5',
                'sandwich_force': self.parent.t_sandwich_force.get() if hasattr(self.parent, 't_sandwich_force') else '15790',
                'max_area_force': self.parent.t_max_area_force.get() if hasattr(self.parent, 't_max_area_force') else '-2.0',
            ...
            if self.parent.sensor_data_window_instance and self.parent.sensor_data_window_instance.sensor_window.winfo_exists():
                try:
                    sdw = self.parent.sensor_data_window_instance
                    ...
                    self.parent.update_status_message(f"Warning: Could not save all sensor settings: {sensor_err}")
            
            # Save to file in the same directory as Prince_Segmented.py
            state_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'prince_gui_state.json')
    ...
    def load_gui_state(self):
        ...
        try:
            ...
            if 'enable_sandwich_precalib' in state:
                self.parent.enable_sandwich_precalib.set(state['enable_sandwich_precalib'])
            
            if 'enable_adaptive_sandwich' in state:
                self.parent.enable_adaptive_sandwich.set(state['enable_adaptive_sandwich'])
            
            if 'enable_scaled_force_sandwich' in state:
                self.parent.enable_scaled_force_sandwich.set(state['enable_scaled_force_sandwich'])
            
            if 'sandwich_gap' in state and hasattr(self.parent, 't_sandwich_gap'):
                self.parent.t_sandwich_gap.delete(0, END)
                self.parent.t_sandwich_gap.insert(0, state['sandwich_gap'])
            
            if 'sandwich_force' in state and hasattr(self.parent, 't_sandwich_force'):
                self.parent.t_sandwich_force.delete(0, END)
                self.parent.t_sandwich_force.insert(0, state['sandwich_force'])
            ...
            if state.get('sensor_window_open', False):
                if not (self.parent.sensor_data_window_instance and self.parent.sensor_data_window_instance.sensor_window.winfo_exists()):
                    self.parent.win.after(100, self.parent.open_sensor_panel)
            
            if state.get('exp_conditions_window_open', False):
                if not (self.parent.exp_conditions_window and self.parent.exp_conditions_window.window.winfo_exists()):
                    self.parent.win.after(200, self.parent.open_exp_conditions_window)
            ...
            if 'sensor_settings' in state and state['sensor_settings']:
                ...
                self.parent._pending_sensor_settings = state['sensor_settings']
                # Try to apply if sensor window is already open
                if self.parent.sensor_data_window_instance and self.parent.sensor_data_window_instance.sensor_window.winfo_exists():
                    self.parent.win.after(300, self.parent._apply_sensor_settings)
    ...
    def trigger_post_print_analysis(self):
        ...
        try:
            ...
            if (hasattr(self.parent, 'exp_conditions_window') and self.parent.exp_conditions_window and 
                hasattr(self.parent.exp_conditions_window, 'current_print_dir') and 
                self.parent.exp_conditions_window.current_print_dir):
                analysis_dir = str(self.parent.exp_conditions_window.current_print_dir)
