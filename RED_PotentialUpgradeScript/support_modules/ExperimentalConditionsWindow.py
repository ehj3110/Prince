...
class ExperimentalConditionsWindow:
    ...
    def _build_ui(self):
        ...
        # Notes text box
        notes_label = ttk.Label(fields_frame, text="Notes:")
        notes_label.grid(row=row, column=0, sticky=(tk.W, tk.N), pady=2)
        
        notes_text = tk.Text(fields_frame, width=50, height=4, wrap=tk.WORD)
        notes_text.grid(row=row, column=1, sticky=tk.W, padx=(5, 0), pady=2)
    ...
    def _write_conditions_to_csv(self):
        ...
        data = {
            ...
            'Notes': self.notes_text.get('1.0', tk.END).strip() or 'N/A',
