...
# Try to import Phidget22, use mocks if not available
try:
    ...
    print("Phidget22 not installed - ForceGaugeManager will use mock mode")
    PHIDGET_AVAILABLE = False
    
    # Mock Phidget classes
    ...
    class VoltageRatioInput:
        ...
        def openWaitForAttachment(self, timeout):
            if not PHIDGET_AVAILABLE:
                raise PhidgetException("Mock: Phidget22 not available")
        def getAttached(self):
            return self.attached
        def close(self):
            self.attached = False
        def setBridgeGain(self, gain):
            pass
        def setDataInterval(self, interval):
            pass
        def setBridgeEnabled(self, enabled):
            pass
