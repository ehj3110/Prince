...
class AdhesionMetricsCalculator:
    ...
    def __init__(self, 
        ...
        Args:
            smoothing_window (int): Savitzky-Golay window length (must be odd)
    ...
    def _apply_smoothing(self, force_data: np.ndarray) -> np.ndarray:
        ...
        try:
            # Use current methodology: Savitzky-Golay filter
    ...
    def _calculate_metrics(self, 
        ...
        # Step 1: Calculate baseline (post-propagation method)
        baseline = self._calculate_baseline(smoothed_force)
        results['baseline_force'] = baseline
        
        # Step 2: Find peak force
        ...
        # Step 3: Find pre-initiation start
        ...
        # Step 4: Find propagation end using second derivative maximum
    ...
    def _calculate_baseline(self, smoothed_force: np.ndarray) -> float:
        """
        Calculate baseline using post-propagation method (last 25% of data).
        """
        baseline_window = max(25, len(smoothed_force) // 4)
        baseline_region = smoothed_force[-baseline_window:]
    ...
    def _find_propagation_end_second_derivative(self, 
        ...
        try:
            ...
            first_derivative = np.gradient(smoothed_force, times)
            ...
            prop_end_idx = peak_idx + max_second_deriv_idx
