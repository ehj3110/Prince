...
# To run this application: double-click the bash file on the desktop
...
'''
        ...
        self.lbl0 = Label(win, text='Rush DLP', font='Helvetica 50 bold')
        ...
        self.lbl15 = Label(win, text='Estimate Time: ∞ min')
        self.lbl16 = Label(win, text='Uplift Speed (um/s)')
        self.lbl17 = Label(win, text='Layer Rest Time(s) After Uplifting')
        ...
        self.t16.place(x=240, y=330)
        self.lbl16.place(x=240, y=310)
        self.t17.place(x=240, y=380)
        self.lbl17.place(x=240, y=360)
        ...
        # Generation of Clickable buttoms
    ...
    def run(self):
        ...
        # Initilize the DLP Lightengine output power
        ...
        while '4202508' != self.my_ensemble.write_read('AXISSTATUS(Z, DATAITEM_AxisStatus)'):
            time.sleep(0.2)
        # Adjust system recursive limit value
    ...
    def _(self, idx):
        ...
        ## Turn of projection image
        cv2.imshow(self.window_name, self.black_image)
        # self.time1 = time.time()
        cv2.waitKey(1)
        #This is for when we wanna wait before moving
        #time.sleep(float(self.t16.get()))
        ##Call A3200 to move a layer's distance with a defined speed. Must convert to micron/second
        self.my_ensemble.write_read(
            'MOVEINC Z {} {}'.format((self.thickness[idx] * -1) / 1000, float(self.t16.get())/1000))
        ## Change the visual effect of layer count on the Rush GUI
        time.sleep(float(self.t17.get()))
