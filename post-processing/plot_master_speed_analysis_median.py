...
Uses MEDIAN instead of MEAN for averaging data points at each speed.
...
def calculate_median_per_condition(df, speed_col, value_col, group_col):
    ...
    for condition in df[group_col].unique():
        ...
        # Group by speed and calculate median and MAD (median absolute deviation)
...
def plot_master_analysis(master_folder, output_path=None):
    """Create master plot with all test conditions using MEDIAN."""
    ...
    pf_data = calculate_median_per_condition(df, speed_col, peak_force_col, 'condition_label')
    woa_data = calculate_median_per_condition(df, speed_col, woa_col, 'condition_label')
    pd_data = calculate_median_per_condition(df, speed_col, peel_dist_col, 'condition_label')
    pr_data = calculate_median_per_condition(df, speed_col, 'abs_retraction', 'condition_label')
    
    # Create figure
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Master Speed Analysis - All Test Conditions (MEDIAN)', fontsize=18, fontweight='bold')
    ...
    for condition, data in pd_data.items():
        ...
        ax3.scatter(data['speeds'], data['medians'], c=color, alpha=0.25, s=80,
                   edgecolors='black', linewidth=1, label=condition, zorder=5)
        if len(data['speeds']) >= 3:
            x_fit, y_fit, _ = create_polynomial_fit(data['speeds'], data['medians'], degree=2)
    ...
    for condition, data in pr_data.items():
        ...
        ax4.scatter(data['speeds'], data['medians'], c=color, alpha=0.25, s=80,
                   edgecolors='black', linewidth=1, label=condition, zorder=5)
        if len(data['speeds']) >= 3:
            x_fit, y_fit, _ = create_polynomial_fit(data['speeds'], data['medians'], degree=2)
    ...
    if output_path is None:
        output_path = Path(master_folder) / "MASTER_speed_analysis_MEDIAN.png"
    
    plt.savefig(output_path, dpi=200, bbox_inches='tight')
    print(f"\nMaster plot (MEDIAN) saved to: {output_path}")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python plot_master_speed_analysis_median.py <path_to_V17Tests_folder>")
        return
