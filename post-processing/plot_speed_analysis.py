...
def calculate_mean_values(df, speed_col, value_col):
    """
    Calculate mean values for each unique speed.
    ...
    Returns:
        ...
        mean_values: Array of mean values for each speed
    ...
    grouped = df.groupby(speed_col)[value_col].agg(['mean', 'std', 'count'])
    ...
    return unique_speeds, mean_values, std_values, all_speeds, all_values
...
def plot_speed_analysis(csv_path, output_path=None):
    ...
    Calculates mean values for each speed and plots with smooth lines.
    ...
    print("\nCalculating mean values for each speed...")
    
    # Peak Force
    speeds_pf, mean_pf, std_pf, all_speeds_pf, all_pf = calculate_mean_values(df, speed_col, peak_force_col)
    print(f"  Peak Force: {len(speeds_pf)} unique speeds")
    
    # Work of Adhesion
    speeds_woa, mean_woa, std_woa, all_speeds_woa, all_woa = calculate_mean_values(df, speed_col, woa_col)
    print(f"  Work of Adhesion: {len(speeds_woa)} unique speeds")
    
    # Peel Distance
    speeds_pd, mean_pd, std_pd, all_speeds_pd, all_pd = calculate_mean_values(df, speed_col, peel_dist_col)
    ...
    speeds_pr, mean_pr, std_pr, all_speeds_pr, all_pr = calculate_mean_values(df, speed_col, 'abs_retraction')
    ...
    x_fit_pf, y_fit_pf, coeffs_pf = create_polynomial_fit(speeds_pf, mean_pf, degree=2)
    x_fit_woa, y_fit_woa, coeffs_woa = create_polynomial_fit(speeds_woa, mean_woa, degree=2)
    x_fit_pd, y_fit_pd, coeffs_pd = create_polynomial_fit(speeds_pd, mean_pd, degree=2)
    x_fit_pr, y_fit_pr, coeffs_pr = create_polynomial_fit(speeds_pr, mean_pr, degree=2)
    
    # Create figure with 4 subplots (2x2 grid)
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('Speed Analysis - Adhesion Metrics vs. Peel Speed', fontsize=16, fontweight='bold')
    ...
    # Mean values (more visible)
    ax1.scatter(speeds_pf, mean_pf, c=color_scheme['peak_force'], alpha=0.6, s=100, 
                edgecolors='black', linewidth=1, label='Mean values', zorder=5)
    # Polynomial fit line
    ...
    ax2.scatter(all_speeds_woa, all_woa, c=color_scheme['work_adhesion'], alpha=0.1, s=50,
                edgecolors='none', label='Individual layers')
    # Mean values (more visible)
    ax2.scatter(speeds_woa, mean_woa, c=color_scheme['work_adhesion'], alpha=0.6, s=100,
                edgecolors='black', linewidth=1, label='Mean values', zorder=5)
    # Polynomial fit line
    ...
    ax3.scatter(all_speeds_pd, all_pd, c=color_scheme['peel_distance'], alpha=0.1, s=50,
                edgecolors='none', label='Individual layers')
    # Mean values (more visible)
    ax3.scatter(speeds_pd, mean_pd, c=color_scheme['peel_distance'], alpha=0.6, s=100,
                edgecolors='black', linewidth=1, label='Mean values', zorder=5)
    # Polynomial fit line
    ...
    ax4.scatter(all_speeds_pr, all_pr, c=color_scheme['retraction'], alpha=0.1, s=50,
                edgecolors='none', label='Individual layers')
    # Mean values (more visible)
    ax4.scatter(speeds_pr, mean_pr, c=color_scheme['retraction'], alpha=0.6, s=100,
                edgecolors='black', linewidth=1, label='Mean values', zorder=5)
    # Polynomial fit line
    ...
    if output_path:
        ...
        print(f"\nSpeed analysis plot saved to: {output_path}")
    else:
        plt.show()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python plot_speed_analysis.py <path_to_autolog_metrics.csv>")
        return
    ...
    output_path = csv_path.parent / "speed_analysis.png"
