"""
Batch Process Presentation Data with Comprehensive Analysis
============================================================

Process all Presentation data folders with:
1. Hydrodynamic locking mitigation (200um distance skip)
2. Individual layer analysis and plots
3. Stiffness analysis (dual-regime detection)
4. Master plots across all conditions
5. Scaling analysis (radius vs metrics)

Usage:
    python batch_process_presentation_data.py
"""
...
def process_folder_with_analysis(folder_path, skip_distance_um=200):
    """
    Process a single folder with comprehensive analysis
    ...
    Returns:
        DataFrame with all results including stiffness metrics
    """
    ...
    if all_results:
        ...
        existing_csv = folder_path / "automated_work_of_adhesion.csv"
        ...
        if existing_csv.exists():
            ...
            print(f"\n  Backed up existing CSV")
            
            # Load area information
            print(f"  Loading area information from existing CSV...")
        ...
        df_results.to_csv(existing_csv, index=False)
        
        print(f"\n  [OK] Saved {len(df_results)} layer measurements to {existing_csv.name}")
...
def main():
    ...
    for folder_path in folders:
        ...
        try:
            result = process_folder_with_analysis(folder_path, skip_distance_um=200)
    ...
    if failed_folders:
        ...
        print("\nNext steps:")
        print("  1. Run generate_presentation_master_plots.py for master comparison plots")
        print("  2. Run analyze_presentation_stiffness.py for stiffness analysis")
        print("  3. Run analyze_presentation_scaling.py for scaling relationships")
