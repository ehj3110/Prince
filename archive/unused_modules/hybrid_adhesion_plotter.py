...
class HybridAdhesionPlotter:
    ...
    def __init__(self, 
        ...
        Args:
            figure_size: Tuple of (width, height) in inches - optimized for monitor display
            dpi: Resolution for saved plots - reduced for faster display
    ...
    def _configure_matplotlib_backend(self):
        ...
        if threading.current_thread() != threading.main_thread():
            # Use Agg backend for non-main threads (non-interactive, file-only)
            matplotlib.use('Agg')
            print("DEBUG: Using Agg backend for background plotting")
        else:
            # Main thread can use interactive backend if available
            try:
                matplotlib.use('TkAgg')  # Interactive backend
                print("DEBUG: Using TkAgg backend for interactive plotting")
            except:
    ...
    def plot_from_arrays(self,
        ...
        for i, peak_idx in enumerate(peaks):
            ...
            # Extract layer segment data - REVERT TO RESET TIME BUT FIX THE CONVERSION
            ...
            try:
                ...
                # Convert metrics back to global time indices - FIXED CONVERSION
                ...
                # Debug: Show the actual values used for plotting
                print(f"  DEBUG - Values used for plotting:")
                print(f"    Plot Peak Time: {layer_obj['peak_time']:.3f} s")
                print(f"    Plot Prop End Time: {layer_obj['prop_end_time']:.3f} s")
                print(f"    Plot Prop End Index: {layer_obj['prop_end_idx']}")
        ...
        if save_path:
            try:
                ...
                print(f"Warning: Error saving plot: {e}")
        
        # Step 7: Display the plot only if in main thread
        import threading
        if threading.current_thread() == threading.main_thread():
            try:
                plt.show()
            except Exception as e:
                print(f"Warning: Cannot display plot in current context: {e}")
        else:
            print("Plot created in background thread - not displaying interactively")
            # Close figure to prevent memory leaks in background threads
            plt.close(fig)
    ...
    def _create_layer_object(self, metrics: Dict, peak_idx: int, start_idx: int, end_idx: int,
        ...
        # Convert relative times back to global times - PROPER CONVERSION
        peak_time = time_data[peak_idx]
        peak_force = force_data[peak_idx]
        baseline = metrics['baseline_force']
        
        # Convert metric times to global times properly
        pre_init_global_time = time_data[start_idx] + metrics.get('pre_initiation_time', 0)
        prop_end_global_time = time_data[start_idx] + metrics['propagation_end_time']
        
        # Find closest indices in the FULL time array, not just the segment
        pre_init_idx = np.argmin(np.abs(time_data - pre_init_global_time))
        prop_end_idx = np.argmin(np.abs(time_data - prop_end_global_time))
        
        # Use the global times from the full array
    ...
    def _create_comprehensive_plot(self, time_data: np.ndarray, force_data: np.ndarray, 
        ...
        """Create dynamic visualization with overview + individual layers in 2-column layout."""
        num_layers = len(layers)
        
        # Calculate dynamic layout: Overview + all layers in 2-column format
        # Always have overview, then arrange layers in 2 columns
        total_plots = 1 + num_layers  # Overview + all layers
        rows_needed = (total_plots + 1) // 2  # Ceiling division for 2-column layout
        
        # Adaptive font scaling based on number of plots
        base_title_size = 16
        base_label_size = 10
        if rows_needed <= 2:
            ...
            title_size = base_title_size - 2
            label_size = base_label_size - 1
        else:
            title_size = base_title_size - 4
            label_size = base_label_size - 2
        
        # Create figure with dynamic height based on number of rows
        fig_height = self.figure_size[1] * (rows_needed / 2.0)  # Scale height proportionally
        
        try:
            # Ensure we're using the correct backend for thread safety
            self._configure_matplotlib_backend()
            fig = plt.figure(figsize=(self.figure_size[0], fig_height), dpi=self.dpi)
        except Exception as e:
            print(f"Warning: Figure creation issue, using fallback: {e}")
            matplotlib.use('Agg')  # Force non-interactive backend
            fig = plt.figure(figsize=(self.figure_size[0], fig_height), dpi=self.dpi)
            
        gs = fig.add_gridspec(rows_needed, 2, height_ratios=[1] * rows_needed, width_ratios=[1, 1])
        ...
        # Dynamic subplot positions for all layers
        subplot_positions = []
        # Start from top-right, then continue in 2-column layout
        subplot_positions.append(gs[0, 1])  # Top-right
        
        # Add remaining positions row by row
        for row in range(1, rows_needed):
            subplot_positions.append(gs[row, 0])  # Left column
            if len(subplot_positions) < num_layers + 1:  # +1 because overview takes first position
                subplot_positions.append(gs[row, 1])  # Right column
        
        # Plot all individual layers
        ...
        # Main title with adaptive font size
    ...
    def _plot_overview(self, ax, time_data, force_data, smoothed_force, layers, font_size=10):
        """Plot the complete overview subplot with adaptive font sizing."""
        ...
        ax.set_title('Complete Force Profile - Multiple Layers', fontsize=font_size+4, fontweight='bold')
    ...
    def _plot_individual_layer(self, ax, time_data, force_data, smoothed_force, layer, font_size=10):
        ...
        # Formatting with adaptive font sizes
        ...
        ax.set_title(f'Layer {layer["number"]} - Peeling Stages with Shaded Bands', 
                    fontsize=font_size+3, fontweight='bold', color=color)
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=font_size-1, loc='upper left', ncol=1, framealpha=0.9, 
    ...
    def _add_layer_annotations(self, ax, layer, font_size=10):
        """Add measurement annotations with adaptive font sizing."""
        ...
        ax.text(text_x_position, y_annotation2 - 0.008, 
               ...
               ha='left', fontsize=font_size-1, color='red', fontweight='bold',
